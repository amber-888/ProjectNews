...
public class WC{
    public static class WCMap extends Mapper<LongWritable, Text, Text,IntWritable > {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (it.hasMoreTokens()) {
                String num = it.nextToken();
                String name = it.nextToken();
                String sex = it.nextToken();
                String time = it.nextToken();
                String web = it.nextToken();
                context.write(new Text( web ), one);
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        Path op = new Path("hdfs://192.168.43.20:9000/output7.0");
        if (fs.exists(op)) {
            fs.delete(op, true);
        }
